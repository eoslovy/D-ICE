...
export class Dice extends Phaser.Scene {
    ...
    // 개별 주사위 생성 함수
    private createDice(x: number, y: number, scene: any, duration = 1000, scale: number) {
    ...
    preload() {
        ...
        this.load.image("woodBackground", "assets/background/woodBackground.png");
    ...
    create() {
        ...
        this.add.image(width/2, height/2, 'woodBackground').setDisplaySize(width, height);
        // 버튼 배치 좌표 계산
        ...
        for (let row = 0; row < gridConfig.rows; row++) {
            for (let col = 0; col < gridConfig.columns; col++) {
                // 수정 코드 (중앙 정렬 + 간격 적용)
                const startX = (this.scale.width - (gridConfig.columns - 1) * gridConfig.spacingX) / 2;
                const startY = (this.scale.height - (gridConfig.rows - 1) * gridConfig.spacingY) / 2;
                const x = startX + col * gridConfig.spacingX;
                const y = startY + row * gridConfig.spacingY;
                const scale = 0.6; // 크기 60%로 축소
                
                // 주사위 생성 및 함수 저장
                const rollFunction = this.createDice(x, y, this, this.rollDuration, scale);
        ...
        this.totalSumText = this.add.text(
            ...
            130, 
            '합 : 0', 
            { font: '40px Jua', color: '#FFFFFF' }
        ).setOrigin(0.5);

        // 남은 횟수 표시
        const rollsLeftText = this.add.text(
            this.scale.width/2,
            200,
            '남은 기회 : 3회',
            { font: '30px Jua', color: '#FFD700' }
        ).setOrigin(0.5);

        

        // -------- 굴리기 버튼 --------
        const rollButton = this.add.graphics()
        ...
        const rollText = this.add.text(
            rollButton.x + buttonWidth/2,
            rollButton.y + buttonHeight/2,
            'GO',
            { font: '28px Jua', color: '#FFFFFF', stroke: '#005577', strokeThickness: 3 }
        ).setOrigin(0.5);

        rollButton.setInteractive(new Phaser.Geom.Rectangle(0, 0, buttonWidth, buttonHeight), Phaser.Geom.Rectangle.Contains);

        rollButton.on('pointerover', () => {
            this.tweens.add({ targets: [rollButton, rollText], scale: 1.08, duration: 100, ease: 'Sine.easeOut' });
        });
        rollButton.on('pointerout', () => {
            this.tweens.add({ targets: [rollButton, rollText], scale: 1, duration: 100, ease: 'Sine.easeIn' });
        });

        // -------- 멈추기 버튼 --------
        const stopButton = this.add.graphics()
        ...
        const stopText = this.add.text(
            ...
            'STOP',
            { font: '28px Jua', color: '#FFFFFF', stroke: '#000000', strokeThickness: 3 }
        ).setOrigin(0.5);

        stopButton.setInteractive(new Phaser.Geom.Rectangle(0, 0, buttonWidth, buttonHeight), Phaser.Geom.Rectangle.Contains);

        stopButton.on('pointerover', () => {
            this.tweens.add({ targets: [stopButton, stopText], scale: 1.08, duration: 100, ease: 'Sine.easeOut' });
        });
        stopButton.on('pointerout', () => {
            this.tweens.add({ targets: [stopButton, stopText], scale: 1, duration: 100, ease: 'Sine.easeIn' });
        });
                    ...
                    roll((value: number) => {
                        ...
                        this.time.delayedCall(this.rollDuration, () => {
                            ...
                            if (index === this.diceRollFunctions.length - 1) {
                                this.rollCount++;
                                this.totalSumText.setText(`합 : ${this.currentRollSum}`);
                                rollsLeftText.setText(`남은 기회 : ${3 - this.rollCount}회`);
        ...
        stopButton.on('pointerdown', () => {
            this.scene.start('GameOver', { score: this.currentRollSum, gameType: "Dice"});
        });

        // 필요하다면: 버튼 위에서 커서 모양 변경
        rollButton.on('pointerover', () => this.input.setDefaultCursor('pointer'));
        rollButton.on('pointerout', () => this.input.setDefaultCursor('default'));
        stopButton.on('pointerover', () => this.input.setDefaultCursor('pointer'));
        stopButton.on('pointerout', () => this.input.setDefaultCursor('default'));
    }
