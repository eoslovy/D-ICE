...
export class Roulette extends Scene {
    ...
    create() {
        ...
        // 룰렛 컨테이너
        this.rouletteMeanY = height / 2;
        this.wheel = this.add.container(width / 2, this.rouletteMeanY);
    ...
    private createSidePointers(
        ...
        // 왼쪽 포인터
        const leftPointer = this.add
            .triangle(
                width / 2 - maskWidth / 2 - pointerSize / 2,
                centerY,
                0,
                pointerSize / 2,
                pointerSize,
    ...
    spinWheel() {
        ...
        const diff =
            (targetIndex - currentCenterGameIdx + GAME_TYPES.length) %
            GAME_TYPES.length;
        const rounds = 2;
        const stopIndex =
            ((targetIndex + GAME_TYPES.length - 1) % GAME_TYPES.length) +
            rounds * GAME_TYPES.length;
        this.stopTargetY = this.rouletteMeanY - stopIndex * this.sliceHeight;
        // 초기 속도
    ...
    update(time: number, delta: number) {
        ...
        for (let i = 0; i < this.wheel.length; i++) {
            ...
            if (item.y + this.wheel.y < -this.sliceHeight * 2) {
                item.y += this.sliceHeight * GAME_TYPES.length;
            }
            // 화면 아래로 완전히 사라지면 위로 올림
            if (
                item.y + this.wheel.y >
                this.cameras.main.height + this.sliceHeight * 2
            ) {
                item.y -= this.sliceHeight * GAME_TYPES.length;
            }
        }
        ...
        if (this.spinPhase === "spinning") {
            ...
            const shouldStop =
                diff < this.sliceHeight * 2.5 || this.spinVelocity < 3;
