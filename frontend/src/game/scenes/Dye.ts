...
export class Dye extends Scene {
    ...
    init() {
        ...
        this.markerColors = new Array(this.numMarkers).fill(null).map(() => new Phaser.Display.Color(0,0,0)); // Init with black
    ...
    create() {
        ...
        this.add.graphics()
            .fillGradientStyle(0x42cafd, 0x42cafd, 0x273c75, 0x273c75)
            .fillRect(0, 0, this.cameras.main.width, this.cameras.main.height);
        // --- Palette Setup ---
        const randomPalette = Phaser.Math.Between(1, 4);
        this.dyePalette = this.add.sprite(this.cameras.main.centerX, this.cameras.main.centerY + 180, `dye_pallete_${randomPalette}`); // Adjusted Y
        ...
        this.add.text(targetDisplayX, targetDisplayY - 50, '염색할 색을 맞추자!', { // Increased font, adjusted Y offset
            fontSize: '40px', color: '#ffffff', fontFamily: 'Arial' 
        }).setOrigin(0.5);
        this.targetColorDisplay = this.add.rectangle(
            targetDisplayX, targetDisplayY + 25, 140, 70, this.targetColor.color // Increased size, adjusted Y
        ).setStrokeStyle(3, 0xffffff).setOrigin(0.5); // Increased stroke

        // --- Draggable Markers Container & Their Color Displays ---
        const markerUIDisplayY = targetDisplayY + 120; // Adjusted Y for spacing
        this.markerContainer = this.add.container(this.dyePalette.x, this.dyePalette.y - 70); // Adjusted initial Y
        this.markerContainer.setDepth(2); 

        const markerRelativePositions = [ // Slightly increased spacing for larger markers
            { x: Phaser.Math.Between(-100, 100), y: Phaser.Math.Between(-100, 100) },    
            { x: Phaser.Math.Between(-100, 100), y: Phaser.Math.Between(-100, 100) },  
            { x: Phaser.Math.Between(-100, 100), y: Phaser.Math.Between(-100, 100) }   
        ];
        ...
        for (let i = 0; i < this.numMarkers; i++) {
            const relPos = markerRelativePositions[i] || { x: (i - 1) * 30, y: 0 };
            const markerSprite = this.add.sprite(relPos.x, relPos.y, 'marker').setScale(0.6); // Increased scale
            const currentMarkerTint = markerTintColors[i % markerTintColors.length];
            ...
            const colorRect = this.add.rectangle(colorDisplayX, markerUIDisplayY + 30, 60, 35, 0x000000) // Increased size
                .setStrokeStyle(2, currentMarkerTint).setOrigin(0.5); // Use marker's tint for stroke
        ...
        this.markerContainer.on('drag', (pointer: Phaser.Input.Pointer, dragX: number, dragY: number) => {
            if (!this.gameStarted || this.gameEnded) return;
            this.markerContainer.x = dragX;
            this.markerContainer.y = dragY;
            this.markers.forEach((m, idx) => {
                m.getWorldTransformMatrix().transformPoint(0, 0, this.markerHelperPoint);
                this.updateColorUnderMarker(this.markerHelperPoint.x, this.markerHelperPoint.y, idx);
            });
        });
        ...
        this.input.on('pointerdown', (pointer: Phaser.Input.Pointer) => {
            ...
            const isPointerOverMarkerGroup = gameObjectsUnderPointer.includes(this.markerContainer) || 
                                           this.markers.some(marker => gameObjectsUnderPointer.includes(marker));
            ...
            const isPointerOverPalette = gameObjectsUnderPointer.includes(this.dyePalette);

            if (this.input.pointer1.active && this.input.pointer2.active && isPointerOverPalette && !this.isPinching) {
        ...
        this.input.on('pointermove', (pointer: Phaser.Input.Pointer) => {
            ...
            if (this.isPinching && this.input.pointer1.active && this.input.pointer2.active) {
                const currentDistance = Phaser.Math.Distance.Between(
                    this.input.pointer1.x, this.input.pointer1.y,
                    this.input.pointer2.x, this.input.pointer2.y
                );
                if (this.pinchInitialDistance > 0) {
                    const scaleFactor = currentDistance / this.pinchInitialDistance;
        ...
        this.input.on('pointerup', (pointer: Phaser.Input.Pointer) => {
            ...
            const activePointersCount = (this.input.pointer1.active ? 1 : 0) + (this.input.pointer2.active ? 1 : 0);
    ...
    updateColorUnderMarker(markerWorldX: number, markerWorldY: number, markerIndex: number) {
        if (!this.dyePalette || !this.dyePalette.texture || !this.dyePalette.texture.key ) { // Removed game state check here, can be handled by caller
            if (this.markerColors[markerIndex]) this.markerColors[markerIndex]?.setTo(0,0,0);
            if (this.markerColorDisplays[markerIndex]) this.markerColorDisplays[markerIndex].setFillStyle(0x000000);
            return;
        }
        ...
        const markerWorldPoint = new Phaser.Math.Vector2(markerWorldX, markerWorldY);
        ...
        inverseSpriteWorldMatrix.transformPoint(markerWorldPoint.x, markerWorldPoint.y, localPoint);
        ...
        const textureX = Math.floor(localPoint.x + frame.width * this.dyePalette.originX);
        const textureY = Math.floor(localPoint.y + frame.height * this.dyePalette.originY);

        let pickedColorObject: Phaser.Display.Color | null = null;
        if (textureX >= 0 && textureX < frame.width && textureY >= 0 && textureY < frame.height) {
            try {
                // this.textures.getPixel returns a new Color object or null
                const tempColor = this.textures.getPixel(textureX, textureY, textureKey, frameName);
                if (tempColor) { // tempColor is a Phaser.Display.Color instance
        ...
        if (pickedColorObject) {
            ...
            this.markerColors[markerIndex]?.setTo(pickedColorObject.red, pickedColorObject.green, pickedColorObject.blue);
        } else {
            this.markerColors[markerIndex]?.setTo(0,0,0); 
        }
        
        if (this.markerColorDisplays[markerIndex] && this.markerColors[markerIndex]) {
            this.markerColorDisplays[markerIndex].setFillStyle(this.markerColors[markerIndex]?.color);
        }
    }
    ...
    startGame() {
        ...
        for (let i = 0; i < this.numMarkers; i++) {
            ...
            if (this.markers[i]) { 
                this.markers[i].getWorldTransformMatrix().transformPoint(0, 0, this.markerHelperPoint);
                this.updateColorUnderMarker(this.markerHelperPoint.x, this.markerHelperPoint.y, i);
            }
        }
        console.log(`Target color: R=${this.targetColor.red} G=${this.targetColor.green} B=${this.targetColor.blue}`);
    }
    ...
    endGame() {
        ...
        this.markers.forEach((marker, index) => {
            marker.getWorldTransformMatrix().transformPoint(0, 0, this.markerHelperPoint);
            this.updateColorUnderMarker(this.markerHelperPoint.x, this.markerHelperPoint.y, index);
        });
        ...
        this.time.addEvent({
            delay: 1000,
            callback: () => {
                this.popupText.popupText(`Game Over\nBest Match: ${finalScore}%`, this.cameras.main.centerX, this.cameras.main.centerY, 3000, {
                    fontSize: '48px', color: '#ffffff', stroke: '#000000',
                    strokeThickness: 2, align: 'center', fontFamily: 'Arial', fontStyle: 'bold',
                });
    ...
    getFinalScore(): number {
        if (!this.targetColor || !this.markerColors || this.markerColors.length === 0) {
            return 0;
        }
        let maxMatchPercentage = 0;
        for (const markerAttemptColor of this.markerColors) {
            if (markerAttemptColor) { 
                const r1 = markerAttemptColor.red; // Use .red
                const g1 = markerAttemptColor.green; // Use .green
                const b1 = markerAttemptColor.blue; // Use .blue

                const r2 = this.targetColor.red; // Use .red
                const g2 = this.targetColor.green; // Use .green
                const b2 = this.targetColor.blue; // Use .blue
                ...
                const maxPossibleDifference = 255 * 3;
                
                const currentMatchPercentage = maxPossibleDifference > 0 
                    ? Math.max(0, (1 - (totalDifference / maxPossibleDifference)) * 100)
                    : 0;
    ...
    result(score: number) {
        console.log("Final Score (Best of 3 Markers):", score);
    }
