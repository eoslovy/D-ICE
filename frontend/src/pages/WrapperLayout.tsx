...
export default function WrapperLayout() {
    ...
    const userConnectAndNavigate = (roomCode: string) => {
        ...
        const handleUserReJoin = () => {
            try {
                userWebSocketManager.sendUserReconnect(
                    requestId,
    ...
    const adminConnectAndNavigate = (roomCode: string) => {
        ...
        const handleAdminReJoin = () => {
            try {
                const adminReconnectRes =
                    adminWebSocketManager.sendAdminReconnect(requestId);
                    ...
                    <div className="modal-content p-6 rounded-lg shadow-lg text-center z-10">
                        ...
                        <div className="modal-actions flex justify-center gap-4 mt-4">
                            <button
                                ...
                                onClick={() => {
                                    if (modalType === "user") {
                                        userConnectAndNavigate(
                                            modalRoomCode!
                                        ).handleModalClose(true);
                                    } else if (modalType === "admin") {
                                        adminConnectAndNavigate(
                                            modalRoomCode!
                                        ).handleModalClose(true);
                                    }
                                }}
