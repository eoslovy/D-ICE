...
class UserWebSocketManager extends WebSocketManager<UserReceiveTypeMap> {
    ...
    protected handleMessage(msg: ReceiveMessage): void {
        if (this.isReceiveMessageWithRequestId(msg)) {
            const timeoutId = this.pendingRequests.get(msg.requestId);
            if (timeoutId) {
                clearTimeout(timeoutId);
                this.pendingRequests.delete(msg.requestId);
            } else {
                this.emit("unknown_requestId", msg.requestId, msg);
                return;
            }
        }
    }
    ...
    //             console.log(
    //                 "[UserWebSocketManager] Message received:",
    //                 message
    //             );
