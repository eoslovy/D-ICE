...
import { Trophy, Medal, Crown, Star, Frown, ChevronDown, ChevronUp, Play, ArrowRight } from "lucide-react"
...
export default function Result({ 
  ...
  const rankingData = isFinalView && finalData 
    ? sortedPlayers 
    : data?.roundRanking || []
  ...
  // 비디오 URL 확인
  const firstVideoUrl = !isFinalView && data ? data.firstPlace?.videoUrl || null : null
  const lastVideoUrl = !isFinalView && data ? data.lastPlace?.videoUrl || null : null
  const hasAnyVideo = !!(firstVideoUrl || lastVideoUrl)
  ...
  const topThreePlayers = isFinalView && sortedPlayers.length > 0 
    ...
    <div className="result-container">
      ...
      {isMobile ? (
        <div className="flex flex-col items-center w-full">
          ...
          {!isFinalView && !videoEnded && hasAnyVideo && currentVideoUrl && (
              ...
              <div className="rankings-table">
                ...
                <div className="rankings-body">
                  ...
                  {rankingData.slice(0, Math.min(3, rankingData.length)).map((player) => {
                    ...
                    return (
                      <div key={player.userId || playerRank} className={rankItemClass}>
                        ...
                        {isFinalView && playerRank <= 3 && (
                          <Star
                            className={`
                              ${playerRank === 1 ? "text-yellow-500" : playerRank === 2 ? "text-gray-400" : "text-amber-700"}
                            `}
                            size={20}
                          />
                        )}
                    ...
                    <>
                      {expandedRankings &&
                        rankingData.slice(3, -1).map((player) => (
                          <div key={player.userId || player.rank} className="rank-item">
                  ...
                  {/* 항상 표시되는 꼴등 */}
                  {rankingData.length > 3 && !expandedRankings && (
                    <div key={rankingData[rankingData.length - 1].userId || rankingData.length} className="rank-item">
                      <div className="rank-number">
      ...
      <div className="continue-button">
        ...
        {isFinalView && (
          <div className="flex flex-col items-center space-y-4">
            {goLobby && (
              <button onClick={goLobby} className="btn btn-secondary inline-flex items-center px-4 py-2">
                🔙 선택 화면으로 돌아가기
              </button>
            )}
          </div>
