...
public class GameSessionRegistry {
    ...
    public void broadcastMessage(String roomId, Object message) throws IOException {
        ...
        if (!gameManager.getRooms().containsKey(roomId)) {
            log.warn("[세션 레지스트리] 브로드캐스트 실패 - 존재하지 않는 방 [방ID: {}]", roomId);
            return;
        }
        ...
        for (PlayerDto player : players) {
            try {
                ...
                log.error("[세션 레지스트리] 메시지 전송 실패 [방ID: {}, 사용자ID: {}]", roomId, player.getUserId(), e);
                invalidUserIds.add(player.getUserId());
            }
        }
        
        if (!invalidUserIds.isEmpty()) {
            log.warn("[세션 레지스트리] 유효하지 않은 세션 제거 [방ID: {}, 사용자: {}]", roomId, invalidUserIds);
    ...
    public boolean isRoomEmpty(String roomId) {
        ...
        if (isEmpty) {
            log.info("[세션 레지스트리] 빈 방 확인 [방ID: {}]", roomId);
        }
        return isEmpty;
    }
    ...
    public void logRoomStatus(String roomId) {
        if (!gameManager.getRooms().containsKey(roomId)) {
            log.info("[세션 레지스트리] 방 상태 확인 - 존재하지 않는 방 [방ID: {}]", roomId);
            return;
        }
        ...
        log.info("[세션 레지스트리] 방 상태 [방ID: {}, 플레이어: {}명, 활성세션: {}개]", 
                roomId, roomPlayerIds.size(), openSessionCount);
    }
